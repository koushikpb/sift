 ...
 * lexical = Postgres FTS only — the mode for embedder-less deployments (Vercel functions
 * can't fetch/cache the bge-large query embedder; see app/next.config.mjs's deploy note).
 */
export function makeRetriever(mode: string | undefined): RetrieveFn {
  ...
  if (mode === "lexical") return (q, d, k = 8) => lexicalRetrieve(q, d, k);
